...
export async function List2(req: Request, res: Response) {
  ...
  if (params.id) { row = (await sdb.oneOrNone<DocumentBase>(`${QueryList} AND d.id = '${params.id}'`)); }
  params.order.forEach(el => el.field += (Props[el.field].type as string).includes('.') ? '.value' : '');
      ...
      switch (f.center) {
        case '=': case '>=': case '<=': case '>': case '<':
          ...
          if (typeof f.right === 'object') {
            const field = f.left.split('.')[0];
            if (field !== 'parent' && field !== 'user' && field !== 'company' && f.right && f.right.id) {
              where += `\nAND CAST(JSON_VALUE(doc, N'$."${field}"') AS UNIQUEIDENTIFIER) = '${f.right.id}' `;
              break;
            }
  ...
  if (queryBefore) {
    ...
    query = `SELECT * FROM (${QueryList} AND d.id IN (${query}) d ${orderbyAfter} OPTION (TABLE HINT(d, INDEX ([PK_Documents])))`;
