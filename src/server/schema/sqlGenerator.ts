...
export function generateQueryObject(doc: { [x: string]: any }, type: string) {

  const simleProperty = (prop: string, type: string) => `, d.doc ->> '${prop}' "${prop}"\n`;
  ...
  const tableProperty = (prop: string, value: any) => {
    ...
    const simleProperty = (prop: string, type: string) => `, '${prop}', x."${prop}"\n`;
    ...
    const pattern = `,
      coalesce((SELECT replace(replace(jsonb_agg(j)::TEXT, '{"${prop}":', ''), '}}', '}')::JSONB FROM (
        ...
        FROM
          "Documents" di, jsonb_to_recordset(d.doc->'${prop}') as x (${xTable})
          ${LeftJoin}
        WHERE di.id = d.id) j
      ), '[]') "${prop}"\n`
...
export function generateQueryList(doc: { [x: string]: any }, type: string) {
...
export function generateQueryNew(doc: { [x: string]: any }, type: string) {
